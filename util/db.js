...
for (const v of _dbList) {
	try {
		...
		console.error(
			"[DB] Can't load database '" + v.name + "' in '" + v.path + "'"
		)
	}
}
...
const _validateDbPath = (path) => {
	...
	if (!path.endsWith(".json"))
		throw TypeError("path doesn't point to json file")
}
...
const create = (name, path, initialData = {}) => {
	...
	if (typeof initialData !== "object")
		throw new TypeError("initialData is not object")
